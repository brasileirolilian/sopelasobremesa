...
- [x] T001 Create `_data/categories.yml` with `slug` and `display_name` entries for all current site categories (inspect `site.categories` by running `bundle exec jekyll build` and checking `_site/` or listing post front matter to discover all slugs)
...
- [x] T005 [US1] Add hamburger button to `_includes/header.html` inside `.header-left`: `<button id="hamburger-btn" class="hamburger-btn" aria-label="Abrir menú de categorías" aria-expanded="false">` containing a 3-line SVG hamburger icon (three `<line>` elements or three `<rect>` elements, 24×24 viewBox, stroke `currentColor`).
- [x] T006 [US1] Include `hamburger-menu.html` in `_includes/header.html` — add `{% include hamburger-menu.html %}` immediately before the closing `</header>` tag (outside `.container` div so the sidebar spans full viewport width).
- [x] T007 [US1] Load `hamburger-menu.js` in the site — add `<script src="{{ '/assets/js/hamburger-menu.js' | relative_url }}" defer></script>` to `_includes/head.html` (or to `_layouts/default.html` before `</body>` — check where other script tags are placed in the layout).
...
- [x] T009 [US2] Remove the `<section class="categories-section">` block from `_layouts/home.html` (lines 74–84: from `<section class="categories-section">` through its closing `</section>`). Do NOT remove the `.categories-section` SCSS in `_components.scss` — it is still used by other pages.
...
- [x] T011 [US3] Move `.social-links-header` block from `.header-left` to `.header-right` in `_includes/header.html`. After this change, `.header-left` contains: hamburger button (from T005) + search include; `.header-right` contains: social icons only.
- [x] T012 [US3] Move `{% include search.html %}` from `.header-right` to `.header-left` in `_includes/header.html` (place it after the `#hamburger-btn` button). Wrap in a `<div class="header-search-wrapper">` if needed for layout control.
- [x] T013 [US3] Update `.header-left` SCSS in `assets/css/_components.scss` to add `gap: 12px; align-items: center;` so hamburger button and search field sit side by side. Verify `.header-right` also has `align-items: center`.
...
- [x] T018 Accessibility check: verify `#hamburger-btn` has `aria-expanded="false"` when closed and `"true"` when open (update JS in `hamburger-menu.js` to toggle `aria-expanded` on open/close). Verify `role="dialog"` and `aria-modal="true"` on sidebar element.
